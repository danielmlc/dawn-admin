    ...
    <el-select 
        ...
        @change="_change">
            <el-option v-for="item in data" 
                ...
                <div v-for="(listitem,index) in lists"  :key="index" style="height:15px;color: #8492a6; font-size: 12px" >
                    {{ item[listitem]}}
                </div>
            </el-option>
    </el-select>
    ...
    methods:{
        loadAll() {
           ...
           if(this.apiConf.url){
                ...
                //处理值为空的情况
                let arr=this.apiConf.query.filter((i)=>{!(i.op==='EQ'&&i.value==='')})
