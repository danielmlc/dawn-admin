...
<div class="form">
  ...
  <el-row class="tips">
      <yl-alert
        v-if="formConf.alert"
        :title="formConf.alert.title" 
        :description="formConf.alert.description" 
        :type="formConf.alert.type" 
        :showIcon="formConf.alert.showIcon" 
        :closable="formConf.alert.closable" 
        :closeText="formConf.alert.closeText" 
        :style="formConf.alert.style"
      >
      </yl-alert>
  </el-row>
    <el-form :model="formModel.order" status-icon ref="dynamicFormConf" label-width="100px" >
  ...
  export default {
    ...
    methods: {
      ...
      _formInitial(){
            ...
            //数据绑定好后拦截处理
            if(this.baseInfoConf.moduleType!=2){
                this._beforeDataRender();
            }
            //字段重复验证处理
